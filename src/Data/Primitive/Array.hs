...
primitive_ :: PrimMonad m s => (State# s -> State# s) -> m ()
{-# INLINE primitive_ #-}
...
newArray :: PrimMonad m s => Int -> a -> m (MutableArray s a)
{-# INLINE newArray #-}
...
readArray :: PrimMonad m s => MutableArray s a -> Int -> m a
{-# INLINE readArray #-}
...
--
-- /Note:/ this function does not do bounds checking.
writeArray :: PrimMonad m s => MutableArray s a -> Int -> a -> m ()
{-# INLINE writeArray #-}
...
unsafeFreezeArray :: PrimMonad m s => MutableArray s a -> m (Array a)
...
unsafeThawArray :: PrimMonad m s => Array a -> m (MutableArray s a)
...
copyArray :: PrimMonad m s
          => MutableArray s a    -- ^ destination array
...
--
-- /Note:/ this function does not do bounds or overlap checking.
copyMutableArray :: PrimMonad m s
          => MutableArray s a    -- ^ destination array
          -> Int                             -- ^ offset into destination array
          -> MutableArray s a    -- ^ source array
          -> Int                             -- ^ offset into source array
          -> Int                             -- ^ number of elements to copy
          -> m ()
...
-- /Note:/ The provided array should contain the full subrange
-- specified by the two Ints, but this is not checked.
cloneMutableArray :: PrimMonad m s
        => MutableArray s a -- ^ source array
        -> Int                          -- ^ offset into destination array
        -> Int                          -- ^ number of elements to copy
        -> m (MutableArray s a)
